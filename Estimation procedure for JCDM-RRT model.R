#-------------------Step1：read data----------------------------#
#Q matrix
Q <- data.matrix(read.csv("real data/4  导入的-第一组-Q矩阵-12题.csv"));
#response
PISA2015_math_response <- read.csv("real data/1  2018阅读-第一组-作答-12题-2003人.csv")
Y <- data.matrix(PISA2015_math_response)
#RTs
PISA2015_math_time <- read.csv("real data/3  2018阅读-第一组-总时间-12题-2003人.csv")
for(f in 1:nrow(Y)){
  for(e in 1:ncol(Y)){
...
#number of actions
PISA2015_math_Action <- read.csv("real data/2  2018阅读-第一组-动作次数-12题-2003人.csv")
